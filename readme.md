...
- `vrp/config`：配置默认值、配置文件定位、加载、校验和数据结构。
- `vrp/workbook`：估值文件发现、格式过滤和工作表读取。
- `vrp/parsing`：单元格取值、类型转换、公式、映射以及持仓和产品解析。
- `vrp/storage`：JSON 文件输出、数据库写入、批次计算和表结构辅助方法。
- `vrp/cli`：主程序和辅助工具的命令行入口。
- `vrp/excel`：保留原有导入路径，并组织完整的解析和输出流程。
